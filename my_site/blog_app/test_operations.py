...
class HealthTests(TestCase):
    ...
    def test_unavailable_database_returns_generic_503(self):
        with patch("blog_app.operations.Post.objects.exists", side_effect=OperationalError("private-diagnostic-detail")):
            with self.assertLogs("blog_app.operations", level="WARNING"):
                response = self.client.get(reverse("health"))
