...
(() => {
    ...
    const snapshot = () => ({
        ...
        personal_tags: form.elements.personal_tags.value,
    });
    const baseline = snapshot();
    let recoverySnapshot = JSON.stringify(baseline);
    let recoveryVersion = Number(form.dataset.recoveryVersion);
    let timer;
    let pending = null;
    let conflict = false;
    let recoveryMessage = "";
    ...
    const autosave = async () => {
        ...
        if (pending || conflict || submitting || serialized === recoverySnapshot) return;
        status.textContent = "Backing up…";
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 10000);
        pending = fetch(form.dataset.autosaveUrl, {
            signal: controller.signal,
            method: "POST", credentials: "same-origin",
            headers: {"Content-Type": "application/json", "X-CSRFToken": form.elements.csrfmiddlewaretoken.value},
            body: JSON.stringify({data, version: recoveryVersion, note_version: form.elements.note_version.value}),
        });
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(result.error || "Backup failed. Save your note before leaving.");
            }
            recoveryVersion = result.version;
            form.elements.recovery_version.value = recoveryVersion;
            recoverySnapshot = serialized;
            recoveryMessage = "Private recovery copy saved";
            recoveryFailed = false;
            retryDelay = 1800;
        } catch (error) {
            recoveryFailed = true;
            retryDelay = 10000;
            recoveryMessage = conflict ? error.message : "Backup failed. Save your note before leaving.";
        } finally {
    ...
    const update = () => {
        ...
        clearTimeout(timer);
        if (dirty && !submitting && !conflict && JSON.stringify(snapshot()) !== recoverySnapshot) timer = setTimeout(autosave, retryDelay);
        const backedUp = JSON.stringify(snapshot()) === recoverySnapshot;
        const label = submitting ? "Saving…" : conflict || recoveryFailed || backedUp ? recoveryMessage || (dirty ? "Unsaved changes" : "No unsaved changes") : "Unsaved changes";
    ...
    const connect = (instance) => {
        ...
        const ready = () => {
            ...
            recoverySnapshot = JSON.stringify(baseline);
            update();
        };
